...
$oldProjectFile = Join-Path $Destination "Personalization.csproj"
...
foreach ($file in $filesToProcess) {
    ...
    if ($content -match "Personalization" -or $content -match "Personalization") {
        # Case sensitive replacements
        $content = $content -replace "Personalization", $NewAppName
        $content = $content -replace "Personalization", $NewAppName.ToLower()
